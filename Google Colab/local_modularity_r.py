...
def compute_r(graph, community, boundary, new_node):
	"""computes the local modularity R if the new node joins the community
	...
	for node in new_boundary:
		for neigh in graph.neighbors(node):
			if neigh in new_community:
				inner_edges += 1
			elif not neigh in new_boundary:
				outer_edges += 1

	return inner_edges / (inner_edges + outer_edges)	


def find_the_best_neighbor(mod_r):
	"""finds the best node from the neighborhood to join the community based on highest modularity R

	Args:
		mod_r ([dict]): [keys: nodes in neighborhood, values: the corresponding modularity R]

	Returns:
		[int]: [a node from the neighborhood]
	"""
	neighbors = list(mod_r.keys())
	best_neigh = neighbors[0]
	for i in range(1, len(neighbors)):
		if mod_r[neighbors[i]] > mod_r[best_neigh]:
