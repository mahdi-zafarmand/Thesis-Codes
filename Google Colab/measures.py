...
def modularity(graph, com2nodes, la=1.0, weight='weight'):
    """calculates modularity of the given graph based on Newman's formula
...
def NMI(node2com_true_filename, node2com_pred):
    """calculates the NMI score of the detected communities

    Args:
        node2com_true_filename ([str]): [the file name of the ground-truth community information]
        node2com_pred ([dict]): [a map of nodes to their corresponding community indices]
    ...
    with open(node2com_true_filename, 'r') as file:
        ...
        for line in lines:
            line = line.split()
            node2com_true[int(line[0])] = int(line[1])

    labels_true = list()
    labels_pred = list()

    nodes = [x for x in node2com_true.keys() if x in node2com_pred.keys()]

    for key in sorted(nodes):
        labels_true.append(node2com_true[key])
        labels_pred.append(node2com_pred[key])
    
    # print('The result is for '+str(len(nodes)) + '/' + str(len(node2com_true)) + ' of all nodes.')
