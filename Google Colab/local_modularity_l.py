...
def compute_L(graph, community, boundary, new_node):
	"""computes the local modularity L (L-metric) if the new node joins the community
	...
	Args:
		...
		new_node ([int]): [the expected node to join the community]
	...
	return (L_in / len(new_community)) / (L_ex / len(new_boundary))


def compute_L2(graph, community, boundary, extra_node):
	"""computes the local modularity L (L-metric) if the extra node leaves the community

	Args:
		graph ([nx.Graph]): [the given network]
		community ([list]): [list of nodes that are already discovered]
		boundary ([list]): [group of nodes in the community that has at least one neighbor outside of it]
		extra_node ([int]): [the expected node to leave the community]

	Returns:
		[float]: [the modularity R if the new node joins the community]
	"""
	new_boundary = list(boundary)
	new_community = list(community)
	new_community.remove(extra_node)
	if extra_node in new_boundary:
		new_boundary.remove(extra_node)
...
def find_the_best_neighbor(mod_l):
	"""finds the best node from the neighborhood to join the community based on highest modularity L

	Args:
		mod_l ([dict]): [keys: nodes in neighborhood, values: the corresponding modularity L]

	Returns:
		[int]: [a node from the neighborhood]
	"""
	neighbors = list(mod_l.keys())
	best_neigh = neighbors[0]
	for i in range(1, len(neighbors)):
		if mod_l[neighbors[i]] > mod_l[best_neigh]:
