...
def calc_strength(graph, n1, n2, n3):
	weights = graph[n1][n2].get('weight', 1.0)
	weights += graph[n1][n3].get('weight', 1.0)
	weights += graph[n2][n3].get('weight', 1.0)
	probs = graph[n1][n2].get('prob', 1.0)
	probs *= graph[n1][n3].get('prob', 1.0)
	probs *= graph[n2][n3].get('prob', 1.0)
	return (weights * probs / 3)


def calc_T_in(graph, node, community, Tin_prev):
	increment = 0.0
	neighbors = list(graph.neighbors(node))
	neighbors_in_com = list(set(neighbors).intersection(community))

	for i in range(len(neighbors_in_com) - 1):
		for j in range(i + 1, len(neighbors_in_com)):
			if graph.has_edge(neighbors_in_com[i], neighbors_in_com[j]):
				increment += calc_strength(graph, neighbors_in_com[i], neighbors_in_com[j], node)

	return Tin_prev + increment
...
def calc_T_out(graph, node, community, Tout_prev):
	...
	neighbors = list(graph.neighbors(node))
	neighbors_in_com = list(set(neighbors).intersection(community))
	neighbors_out_com = list(set(neighbors) - set(community))
		...
		for neigh2 in neighbors:
			if neigh1 != neigh2 and graph.has_edge(neigh1, neigh2):
				if not neigh1 in community and not neigh2 in community:
					increment += calc_strength(graph, neigh1, neigh2, node)
				elif neigh1 in community and not neigh2 in community:
					decrement += calc_strength(graph, neigh1, neigh2, node)
